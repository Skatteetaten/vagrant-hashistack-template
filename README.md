...
- `make template_example` runs the example in [template_example/](template_example).
  - `make proxy` uses a consul docker image to run a [connect proxy](https://www.consul.io/commands/connect/proxy) that connects to the `countdash-dashboard`. The dashboard is available at [localhost:9999](http://localhost:9999/).
